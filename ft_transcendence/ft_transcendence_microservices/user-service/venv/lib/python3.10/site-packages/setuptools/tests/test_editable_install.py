...
@pytest.mark.xfail(sys.platform == "darwin", reason="pypa/setuptools
...
class TestPep420Namespaces:
    ...
    def test_namespace_accidental_config_in_lenient_mode(self, venv, tmp_path):
        ...
        parent packages, and make them namespaces (reported in issue 
...
class TestFinderTemplate:
    ...
    def test_combine_namespaces_nested(self, tmp_path):
        ...
        ``package_dir`` as shown in pypa/setuptools
        """
    ...
    def test_intermediate_packages(self, tmp_path):
        ...
        don't exist (see pypa/setuptools
        """
        files = {
            "src": {
                "mypkg": {
...
def test_pbr_integration(tmp_path, venv, editable_opts):
    """Ensure editable installs work with pbr, issue 
    files = {
        "pyproject.toml": dedent(
            """\
            [build-system]
...
def test_encode_pth():
    ...
    content = _encode_pth("tkmilan_ç_utf8")  
