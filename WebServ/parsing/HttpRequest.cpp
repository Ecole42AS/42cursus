...
void HttpRequest::setQueryParameters(const std::map<std::string, std::string>& query_params) { _query_params = query_params; }


// Cleaner to pass from std::vector with linear search O(n) to std::set O(log n) 
// with logarithmic search and also for element unicity
